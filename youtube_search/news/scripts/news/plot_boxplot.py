...
def create_boxplot():
    for i in (Chen0, Chen1, Chen2):
        ...
        with open(f'{DIR}/templates/news/{name_channel_eng}.html', 'w') as ht:
            ht.write(html_str)
...
import os
def create_boxplot():
    for i in os.listdir(f'{DIR}/scripts/news/boxplot'):
        if i == 'chen0.json':
            name_channel = 'BadComedian'
            name_channel_eng = 'BadComedian'
        if i == 'chen1.json':
        ...
        with open(f'{DIR}/scripts/news/boxplot/{i}', 'r') as e:
